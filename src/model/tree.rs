...
use crate::train::tree::{self, RandomForest, RandomForestClassifier, TreeNode};
...
/// Random Forest classifier ensemble model (Gini trees, majority vote)
pub struct ForestClassifierModel {
    pub metadata: ModelMetadata,
    forest: RandomForestClassifier,
}

impl ForestClassifierModel {
    pub fn new(
        forest: RandomForestClassifier,
        num_features: usize,
        num_samples: usize,
        ...
        let metadata = ModelMetadata {
            algorithm: Algorithm::RandomForestClassifier,
            num_features,
            num_samples,
            r_squared: None,
            mse: None,
...
impl MlModel for ForestClassifierModel {
    fn predict(&self, features: &[f64]) -> Result<f64, ModelError> {
        if features.len() != self.metadata.num_features {
            return Err(ModelError::FeatureCountMismatch {
                expected: self.metadata.num_features,
        ...
        Self: Sized,
    {
        ...
        Ok(Self {
            ...
            forest: RandomForestClassifier { trees },
        })
    }
}
